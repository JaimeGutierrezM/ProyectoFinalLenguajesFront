...
object AdminView {
  ...
  def apply(
    ...
    Ajax.get("https://tl7vhlzb-8081.brs.devtunnels.ms/libros")
      .map { xhr =>
        val json = JSON.parse(xhr.responseText).asInstanceOf[js.Array[js.Dynamic]]

        val libros = json.map { obj =>
          PaginaPrincipal.Libro(
            id_libro = obj.id_libro.asInstanceOf[Int],
            nombre = obj.nombre.asInstanceOf[String],
            precio = obj.precio.asInstanceOf[Double],
            id_categoria = obj.id_categoria.toString,
            nombre_categoria = obj.nombre_categoria.asInstanceOf[String],
            nombrepdf = obj.nombrepdf.asInstanceOf[String],
            nombreimagen = obj.nombreimagen.asInstanceOf[String]
          )
        }.toList
      ...
      .recover { case err =>
        dom.console.error("Error cargando libros:", err.toString)
      }
...
onMountCallback { _ =>
  Ajax.get("http://tu-backend/listarLibros").foreach { xhr =>
    val json = JSON.parse(xhr.responseText).asInstanceOf[@@js.Array[js.Dynamic]]
